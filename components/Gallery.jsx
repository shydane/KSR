...
// Placeholder ilustrasi foto (langit + awan + bukit), dipakai berulang.
// Ganti masing-masing <PhotoArt /> di bawah dengan <Image src="..." alt="..." fill />
// begitu ada foto kegiatan asli.
function PhotoArt() {
  return (
...
const leftPhotos = [
  { id: "l1", top: 0, edge: -24, rotate: -9, shape: "shape-a" },
  { id: "l2", top: 165, edge: -52, rotate: 6, shape: "shape-b" },
  { id: "l3", top: 330, edge: -50, rotate: -6, shape: "shape-c" },
  { id: "l4", top: 495, edge: -26, rotate: 8, shape: "shape-a" },
];

const rightPhotos = [
  { id: "r1", top: 40, edge: -24, rotate: 8, shape: "shape-b" },
  { id: "r2", top: 205, edge: -54, rotate: -7, shape: "shape-c" },
  { id: "r3", top: 370, edge: -52, rotate: 9, shape: "shape-a" },
  { id: "r4", top: 535, edge: -24, rotate: -8, shape: "shape-b" },
];
    ...
    <section ref={container} className="gallery" id="gallery">
      <div className="gallery-stage">
        {leftPhotos.map((p) => (
          <div
            key={p.id}
            className={`g-photo g-photo-left ${p.shape}`}
            style={{ top: p.top, left: p.edge }}
        ...
        {rightPhotos.map((p) => (
          <div
            key={p.id}
            className={`g-photo g-photo-right ${p.shape}`}
            style={{ top: p.top, right: p.edge }}
            data-rotate={p.rotate}
          >
            <PhotoArt />
          </div>
        ...
        <div className="gallery-center">
          ...
          <p className="gallery-caption">Momen dari setiap penugasan lapangan kami.</p>
        </div>
      </div>
    </section>
  );
